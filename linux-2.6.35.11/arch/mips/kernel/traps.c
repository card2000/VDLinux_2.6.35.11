...
static void show_backtrace(struct task_struct *task, const struct pt_regs *regs)
{
	...
	do {
		...
		if (pc <= 0) //Checking the Reutrning zero on error condition -- GJ 
			break;
...
static void show_epc_ra_kernel(const struct pt_regs * regs)
{
	...
	int valid_pc = 0, valid_lr = 0;
       ...
       if( (TASK_SIZE <= regs->cp0_epc) && (regs->cp0_epc < (long)high_memory) ){
               valid_pc = 1;
	}
       else{
               valid_pc = 0;
       ...
       if((TASK_SIZE <= regs->regs[31]) && (regs->regs[31] < (long) high_memory)){
               valid_lr = 1;
	}
       else{
               valid_lr = 0;
       ...
       if( valid_pc && addr_epc < TASK_SIZE ){
               addr_epc = TASK_SIZE;
	}

       if( valid_lr && addr_ra < TASK_SIZE ){
               addr_ra = TASK_SIZE;
	}
...
/* VDLinux 1.1.1.x, based VDLP.4.3.1.x default patch No.6,
