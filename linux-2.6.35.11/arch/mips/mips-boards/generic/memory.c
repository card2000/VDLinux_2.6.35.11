...
static int __init EMMAC_setup(char *str)
{
    //printk("LX_MEM= %s\n", str);
    if( str != NULL )
    {
...
early_param("EMAC_MEM", EMMAC_setup);
...
#define MIU1_BASE 0x60000000
#define MIU1_LENGTH  0x10000000
#define MIU1_MAX  MIU1_BASE+MIU1_LENGTH
#define MIU1_RESERVE 0x1000
...
void __init prom_meminit(void)
{
    ...
    // 2011/0930 modify by mstar: Code refine for add_memory_region
    if (linux_memory_length != 0)
	{
		...
		{
                        //printk("Hardcoding linux address for avoiding E-log overlap kernel memory\n");
                        add_memory_region(linux_memory_address, 0x10000000-linux_memory_address, BOOT_MEM_RAM);
			linux_memory3_address = 0x10000000;
			linux_memory3_length =  linux_memory_length -  (0x10000000 - linux_memory_address);
			add_memory_region(0x40000000+linux_memory3_address, linux_memory3_length, BOOT_MEM_RAM);
		}
	}
    if ((linux_memory2_length != 0) ||  ((linux_memory2_address+linux_memory2_length) <= MIU1_MAX))
    {
        printk("X10 dummy write buffer reserved\n");
        // Reserve the tail of MIU1 for L2 catch buffer (4KB) 
        if((linux_memory2_address+linux_memory2_length)>(MIU1_MAX-MIU1_RESERVE))
        {
           linux_memory2_length = MIU1_MAX-MIU1_RESERVE - linux_memory2_address;
