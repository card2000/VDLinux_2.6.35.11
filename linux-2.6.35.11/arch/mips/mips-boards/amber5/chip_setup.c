...
volatile unsigned long miu0_dummy[16];
volatile unsigned long* miu0_dummy_ptr;
volatile unsigned long* miu1_dummy_ptr;
static void dummy_write(void)
{
    //repeat again
    miu0_dummy_ptr[0] = 0x12345678;//jc: dummy write miu0 before fire
    miu1_dummy_ptr[0] = 0x12345678;//jc: dummy write miu1 before fire
    miu0_dummy_ptr[1] = 0x12345678;//jc: dummy write miu0 before fire
    miu1_dummy_ptr[1] = 0x12345678;//jc: dummy write miu1 before fire
    miu0_dummy_ptr[2] = 0x12345678;//jc: dummy write miu0 before fire
    miu1_dummy_ptr[2] = 0x12345678;//jc: dummy write miu1 before fire
    miu0_dummy_ptr[3] = 0x12345678;//jc: dummy write miu0 before fire
    miu1_dummy_ptr[3] = 0x12345678;//jc: dummy write miu1 before fire
    //repeat again
    miu0_dummy_ptr[0] = 0x12345678;//jc: dummy write miu0 before fire
    miu1_dummy_ptr[0] = 0x12345678;//jc: dummy write miu1 before fire
    miu0_dummy_ptr[1] = 0x12345678;//jc: dummy write miu0 before fire
...
inline void _chip_flush_miu_pipe(void)
{
    ...
    *(volatile unsigned int *)(0xbf203104) |= BIT4;
    ...
    dwReadData = *(volatile unsigned int *)(0xbf20317c); //jc: dummy read before check finish status
    dwReadData = *(volatile unsigned int *)(0xbf20317c); //jc: dummy read before check finish status
    dwReadData = *(volatile unsigned int *)(0xbf20317c); //jc: dummy read before check finish status
    do
    {
...
void Chip_L2_cache_wback_inv( unsigned long addr, unsigned long size)
{
	...
	else
	{
        ...
        else // range flush
        {
            ...
            *(volatile unsigned int *)(0xbf203118) = ((addr + size) & 0xFFFF);
            *(volatile unsigned int *)(0xbf20311C) = (((addr + size) >> 16 ) & 0xFFFF);
...
void Chip_Query_L2_Config(void)
{
    ...
    else
    {
        ...
        unsigned long flags;
        unsigned long entrylo1 = (((0x70000000-0x1000)>> 12) << 6) | ((_page_cachable_default >> _CACHE_SHIFT) << 3) | (1<< 2) | (1<< 1) | (1<< 0); // cacheable
        unsigned long entrylo0 = (((0x70000000-0x1000)>> 12) << 6) | (2<<3) | (1<< 2) | (1<< 1) | (1<< 0); // un-cacheable or 0x7
        local_irq_save(flags);
        add_wired_entry(entrylo0, entrylo1, 0xD0000000, PM_4K);
        local_irq_restore(flags);
		miu0_dummy_ptr = (volatile unsigned long*) (((long)(&miu0_dummy))|0x20000000);
		miu1_dummy_ptr = (volatile unsigned long*) 0xD0000000;
		//miu1_dummy_ptr = (volatile unsigned long*) miu0_dummy_ptr;
