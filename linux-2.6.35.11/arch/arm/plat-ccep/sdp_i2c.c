...
#else	// IRQ_SHARED
#  if defined(CONFIG_ARCH_SDP1106)	// 5,6,7 interrupt shared 1.235
	if(irq == IRQ_I2C5) {
		regVal = R_I2C_PEND;
...
static int i2c_check_bus(struct i2c_ctrl_t *pPort)
{
	...
	if(I2CSTAT_BUSY){
		...
		delay = jiffies + HZ/10;	// 100mS delay
		while(time_before(jiffies, delay)) cond_resched();
		...
		printk("[%s] %d status 0x%08x \n", __FUNCTION__, pPort->port, R_I2C_STAT);

		while(I2CSTAT_BUSY && retry) {	// 1.237
// status 
...
// sync condition to false
			...
			if(regVal == 0xB0) { // read mode
				...
				printk("[%s recover] i2c status exception 0x%08x\n",__FUNCTION__, regVal);
				printk("[%s recover] Please check i2c %d clock/data line \n",__FUNCTION__, pPort->port);
				return -EIO;	
			}
...
// Mar,04,2009  tukho.kim
	waitTime = pPacket->subAddrSize + pPacket->dataSize;
...
// 0xFE use at check_bus function 
	...
	if(i2c_check_bus(pPort) < 0) { // 1.237
		printk("[%s] i2c %d line NOT READY !!!!\n", __FUNCTION__, pPort->port);
		up(&pPort->mutex);
		return -EIO;
	}
