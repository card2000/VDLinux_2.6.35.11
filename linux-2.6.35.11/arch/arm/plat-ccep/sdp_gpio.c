...
#define SELECT_GPIO(x)	do { if(x > 0xFF) { x -= 0x100; bMicom = 1;} else {bMicom = 0;} } while(0)
#else
...
struct _sdp_gpio_pull *gpio_pullmap[255] = {NULL, };
...
void sdp_gpio_cfgpull(unsigned int gpionum, unsigned int updown)
{
	...
	if(pullmap[bMicom][gpionum] == NULL)
	{
		...
		printk("Not Exist GPIO (port:%d  pin:%d)\n", port, pin);
	}
	...
	value &= ~(3 << (pullmap[bMicom][gpionum]->regshift));
...
static int gpio_ioctl(struct inode * inode, struct file * file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case GPIO_IOC_WRITE:
			...
			break;
		case GPIO_IOC_READ:
			sIoData.gpiolevel = sdp_gpio_get_value(sIoData.gpioNum);
