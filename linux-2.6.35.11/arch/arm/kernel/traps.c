...
static void show_pc_lr_kernel(const struct pt_regs *regs)
{
    ...
    if((TASK_SIZE <= regs->ARM_pc) &&  (regs->ARM_pc < (long)high_memory)){
         valid_pc = 1;
    }else{
         valid_pc = 0;}
    if((TASK_SIZE <= regs->ARM_lr) &&  (regs->ARM_lr < (long)high_memory)){
         valid_lr = 1;
     }else{
         valid_lr = 0;}

    if( valid_pc && (addr_pc < TASK_SIZE) ){
        addr_pc = TASK_SIZE;}
    if( valid_lr && (addr_lr < TASK_SIZE) ){
        addr_lr = TASK_SIZE;}
...
void show_info(struct task_struct *task, struct pt_regs *regs, unsigned long addr)
{
   ...
   printk(KERN_ALERT "DDR A\n");
   printk(KERN_ALERT "Print 0xFE4085E4 0x%x\n", *((volatile unsigned int*)0xFE4085E4));
   printk(KERN_ALERT "Print 0xFE408624 0x%x\n", *((volatile unsigned int*)0xFE408624));
   printk(KERN_ALERT "Print 0xFE408664 0x%x\n", *((volatile unsigned int*)0xFE408664));
   printk(KERN_ALERT "Print 0xFE4086a4 0x%x\n", *((volatile unsigned int*)0xFE4086a4));

   printk(KERN_ALERT "DDR B\n");	  
   printk(KERN_ALERT "Print 0xFE8085E4 0x%x\n", *((volatile unsigned int*)0xFE8085E4));
   printk(KERN_ALERT "Print 0xFE808624 0x%x\n", *((volatile unsigned int*)0xFE808624));

   printk(KERN_ALERT "DDR C\n");
   printk(KERN_ALERT "Print 0xFE8185E4 0x%x\n", *((volatile unsigned int*)0xFE8185E4));
   printk(KERN_ALERT "Print 0xFE818624 0x%x\n", *((volatile unsigned int*)0xFE818624));   
#endif
...
static void dump_backtrace(struct pt_regs *regs, struct task_struct *tsk)
{
	...
	printk("Backtrace(CPU %d): ", raw_smp_processor_id());
...
void back_trace( void )
{
    ...
    if (in_interrupt())
        regs = kmalloc( sizeof( struct pt_regs ),GFP_ATOMIC);
    else
    ...
    //need to get pid in interrupt context  
    //if ((((regs)->ARM_cpsr & 0xf) == 0))
    //    show_user_stack(current,regs);
