...
//		pos += sprintf(ccrdesc+pos, "%dbyte/loop ", BRST_SIZE_SRC(val) * BRST_LEN_SRC(val));
...
//		pos += sprintf(ccrdesc+pos, "%dbyte/loop ", BRST_SIZE_DST(val) * BRST_LEN_DST(val));
...
/* this func is support burst aligned size transfer*/
		...
		const struct _xfer_spec *pxs)
{
	...
	const u32 org_dstbrst_len = GET_DST_BRST_LEN(pxs->ccr);
	const u32 org_dstbrst_size = 0x1 << GET_DST_BRST_SIZE(pxs->ccr);
	u32 src_load_byte = 0;
	/* if 0 burst, return 0 */
	if(bursts == 0)
		return off;
	...
	if(!dry_run)
		pr_debug("_setup_loops: srcoff= %x, dstoff= %x, bursts= %d\n", srcoff, dstoff, bursts);

	if(srcoff == dstoff)//able src, dst aligned
	{
		if(srcoff != 0)
		{
			ccr = pxs->ccr;
			ccr = _byte_to_ccr_src(align_size, ccr);
			ccr = _byte_to_ccr_dst(align_size - dstoff, ccr);
	...
	else if(srcoff < dstoff)//able src aligned
	{
		if(srcoff == 0)
		{
			/* src aligned */
	...
	else if(srcoff > dstoff)//able dst aligned
	{
		if(dstoff == 0)
		{
			/* dst aligned */
		...
		ccr = pxs->ccr;
		//On this path, the condition "dstoff" could not be false.
		//so, comment dead statement 
//		if(dstoff)
			ccr = _byte_to_ccr_src(align_size*2, ccr);
//		else
//			ccr = _byte_to_ccr_src(align_size, ccr);
//		if(dstoff) 
			ccr = _byte_to_ccr_dst(align_size - dstoff, ccr);
...
//		if(dstoff)
		...
		/* Loop cnt - 1 */
		bursts -= 1;
		src_load_byte += BURST_TO_BYTE(1, ccr);
	}

	const burst_diff = BYTE_TO_BURST(x->bytes, pxs->ccr) - bursts;
	...
	if( burst_diff )
	{
		...
		{
			u32 remain_byte = BURST_TO_BYTE(1, pxs->ccr) - BURST_TO_BYTE(1, ccr);
			ccr = pxs->ccr;
			ccr = _byte_to_ccr_src(remain_byte, ccr);
			ccr = _byte_to_ccr_dst(remain_byte, ccr);
			off += _emit_MOV(dry_run, &buf[off], CCR, ccr);
			off += _emit_LD(dry_run, &buf[off], ALWAYS);
			off += _emit_ST(dry_run, &buf[off], ALWAYS);
		}
	...
	{
		ccr = pxs->ccr;
		if(srcoff) ccr = _byte_to_ccr_src(srcoff, ccr);
		ccr = _byte_to_ccr_dst(dstoff, ccr);
		off += _emit_MOV(dry_run, &buf[off], CCR, ccr);
		
		if(srcoff) off += _emit_LD(dry_run, &buf[off], ALWAYS);
		off += _emit_ST(dry_run, &buf[off], ALWAYS);
	}
	else if(srcoff > dstoff)
	{
		...
		ccr = _byte_to_ccr_src(srcoff, ccr);
		if(!dstoff)
			ccr = _byte_to_ccr_dst(align_size, ccr);
		else if(BRST_LEN(pxs->ccr) == 1)
			ccr = _byte_to_ccr_dst(dstoff, ccr);
		else
			ccr = _byte_to_ccr_dst(dstoff+align_size, ccr);
		...
		const struct _xfer_spec *pxs)
{
	struct pl330_xfer *x = pxs->x;
	int off = 0;
	const int prelength = x->bytes % (BRST_SIZE(pxs->ccr) * BRST_LEN(pxs->ccr));
	...
	/* Setup Loop(s) for pre unaligned length
		if 128byte align, 72byte of 200byte */
	if(!dry_run) pr_debug("prelength %d bytes %d len_align %d\n", prelength, x->bytes, (BRST_SIZE(pxs->ccr) * BRST_LEN(pxs->ccr)));
	if (prelength)
	{
		if(!dry_run) pr_debug("pre length start.\n");
		...
		{
			if(!dry_run) pr_debug("invalid ccr 0x%x\n", _pxs.ccr);
			return INT_MAX;
		}
		int _bursts = BYTE_TO_BURST(prelength, _pxs.ccr);
...
//		off += _emit_WMB(dry_run, &buf[off]);
	...
	x->bytes -= prelength;
	x->src_addr += prelength;
	x->dst_addr += prelength;
...
static int _setup_req_cnt(unsigned dry_run, struct pl330_thread *thrd,
		unsigned index, struct _xfer_spec *pxs)
{
	struct _pl330_req *req = &thrd->req[index];
	struct pl330_xfer *x;
	u8 *buf = req->mc_cpu + req->mc_len;

	const int endsize = _setup_req_end(0, thrd, buf);
	int off = 0;

	if(req->mc_len < endsize) return -EINVAL;

	buf -= endsize;

	PL330_DBGMC_START(req->mc_bus + req->mc_len - endsize);
