...
//printk(KERN_WARNING "amber3_irq_ack(irq=%d)\n",irq);
        if(irq <16)
	{
         tmp = reg_readw(REG_INT_BASE_PA + 0x2c*4);
	    tmp |= (0x01 << irq);
        reg_writew(tmp, REG_INT_BASE_PA + 0x2c*4);
        reg_writew(0x0, REG_INT_BASE_PA + 0x2c*4);
	}
	else if((irq >= 16) && (irq < 32))
	{
         tmp = reg_readw(REG_INT_BASE_PA + 0x2d*4);
	    tmp |= (0x01 << (irq-16));
        reg_writew(tmp, REG_INT_BASE_PA + 0x2d*4);
        reg_writew(0x0, REG_INT_BASE_PA + 0x2d*4);
	}
	else if( (irq >= 32) && (irq < 48))
	{
        tmp = reg_readw(REG_INT_BASE_PA + 0x2e*4);
		tmp |= (0x01) << (irq - 32);
        reg_writew(tmp, REG_INT_BASE_PA + 0x2e*4);
        reg_writew(0x0, REG_INT_BASE_PA + 0x2e*4);
	}
	else if( (irq >= 48) && (irq < 64))
	{
        tmp = reg_readw(REG_INT_BASE_PA + 0x2f*4);
		tmp |= (0x01) << (irq - 48);
        reg_writew(tmp, REG_INT_BASE_PA + 0x2f*4);
        reg_writew(0, REG_INT_BASE_PA + 0x2f*4);
        }
	/*
...
void __iomem *gic_cpu_base_addr=(0xFC000000 + 0x0100);
void __iomem *gic_dist_base_addr=(0xFC000000 + 0x1000);
