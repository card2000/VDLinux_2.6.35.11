 ...
 * @last_block_in_bio: last block of the nth bio when (n+1)th  iteration is going on.
 * @bh:         buffer head pointer used for block mapping.
 *
 * map the block to the buffer heads in the page and then fill the page with data.
 * keep on returning bio pointer so that bio vector keeps on growing for contiguous 
 * blocks. In case blocks are not contiguous just submit bio for input and start 
 * all over again.
 *
 * Return bio pointer
 */
static struct bio * do_ntfs_read_block(struct file *file, struct bio *bio, struct page *page,unsigned nr_pages,
		sector_t *last_block_in_bio, struct buffer_head *bh)
{
	loff_t i_size;
	VCN vcn;
	LCN lcn;
	...
	ntfs_inode *ni;
	ntfs_volume *vol;
	runlist_element *rl = NULL; // in case goto confused, runlist should be NULL first.
	...
	for (page_idx = 0; page_idx < nr_pages; page_idx++) {
		...
		if (!add_to_page_cache_lru(page, mapping, page->index, GFP_KERNEL)) {                     
			...
			if (NInoNonResident(ni) &&
				...
				bio = do_ntfs_read_block(file, bio, page, nr_pages-page_idx, &last_block_in_bio, &map_bh);
			else
