        ...
        xfs_split_data_t *split_data)
{
	 ...
	 * */
        ...
        xfs_trans_ijoin(split_data->tp, split_data->ip, XFS_ILOCK_EXCL | XFS_IOLOCK_EXCL);
        xfs_trans_log_inode(split_data->tp,split_data->ip,XFS_ILOG_ALL);
        xfs_trans_ihold(split_data->tp, split_data->ip);
        ...
        if (error) {
                ...
                xfs_ichgtime(split_data->ip, XFS_ICHGTIME_MOD | XFS_ICHGTIME_CHG);
                error = xfs_trans_commit(split_data->tp, XFS_TRANS_RELEASE_LOG_RES);
        }

        xfs_iunlock(ip, XFS_ILOCK_EXCL | XFS_IOLOCK_EXCL);
        xfs_iunlock(split_data->ip, XFS_ILOCK_EXCL | XFS_IOLOCK_EXCL);
        return error;
}
/*
