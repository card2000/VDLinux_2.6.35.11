        ...
        case XFS_IOC_SPLIT_FILE:
                {
                        ...
                        filp_split = filp_open(spt.filename,O_CREAT | O_EXCL | O_LARGEFILE, 0666);
                        if(IS_ERR(filp_split))
                                return -XFS_ERROR(EEXIST);
                        else
                        {
                                xfs_bmap_free_t flist; // Free list for leaf block extents
                                /* populate split_data */
                                split_inode = filp_split->f_dentry->d_inode;
