...
#include <SBKN_rsa.h>
#include <SBKN_bignum.h>
extern cc_u8 GN_Coredump[RSA_KEYBYTE_SIZE];
#endif

#ifdef CONFIG_BINFMT_ELF_COMP
/* VDLinux 1.1.1.x, based VDLP.4.3.1.x default patch No.10, 
   ultimate coredump v0.4, SP Team 2009-05-13 */
...
#ifndef CONFIG_ENCRYPT_COREDUMP
...
#ifdef CONFIG_ENCRYPT_COREDUMP
static int set_gzip_header(struct file *file,struct coredump_params *cprm);
static int set_gzip_tailer(struct file *file, unsigned long *crc, unsigned long *uncomp_size,struct coredump_params *cprm);
static int compress_coredump(struct file *file, unsigned char *src, unsigned char *comp_buf,
        loff_t src_len, unsigned long *crc, u32 *is_z_finish,struct coredump_params *cprm);
static int coredump_alloc_workspaces(z_stream *def_strm);
static void coredump_free_workspaces(z_stream *def_strm);
#else
...
void b64_encode( cc_u8 *input, cc_u8 *output, int insize )
{
...
static void encrypt_write(struct file *file,const char *tmp,int len_available,int len_write,struct coredump_params *cprm)
{
	...
	int a = 0;
        ...
        while(len_available)
        {
                for (i = cprm->bufs_index; i < ENCODE_IN_SIZE; i++)
                {
                        cprm->buf_encode_in[i] = *tmp;
                        tmp++;
                        len_available--;
                        cprm->bufs_index++;
                        if (!len_available)
                ...
                if (cprm->bufs_index >= len_write)
                {
			/* b64_encode function is called as it is required to remove padding
 			 * noise, etc. It is also mandatory as it is based on decrypt tool,
 			 * which is using b64_decode fucntions to track end of data, padding
 			 * etc. Therefore it is satisfying decrypt tool algorithm.
 			 */
			b64_encode(cprm->buf_encode_in,cprm->buf_encode_out,cprm->bufs_index);
...
{   
	...
	gzip_hdr.os = 3;
#ifdef CONFIG_ENCRYPT_COREDUMP
	encrypt_write(file,(char *)&gzip_hdr,sizeof(gzip_header_t),ENCODE_IN_SIZE,cprm);
#else
	/* gzip_header_t write */
...
#ifdef CONFIG_ENCRYPT_COREDUMP
	encrypt_write(file,coredump_filename,strlen(coredump_filename)+1,ENCODE_IN_SIZE,cprm);
#else
...
#ifdef CONFIG_ENCRYPT_COREDUMP
	encrypt_write(file,(char *)crc,sizeof(*crc),ENCODE_IN_SIZE,cprm);
	encrypt_write(file,(char *)uncomp_size,sizeof(*uncomp_size),1,cprm);
#else
...
#ifdef CONFIG_ENCRYPT_COREDUMP
static int create_workspace(struct coredump_params *cprm)
{
	cc_u32 RSA_KeyByteLen = RSA_KEYBYTE_SIZE;
	cprm->pbbuf_mod = (cc_u8*)kmalloc((SDRM_RSA_ALLOC_SIZE),GFP_KERNEL);
	cprm->pbbuf_exp = (cc_u8*)kmalloc((SDRM_RSA_ALLOC_SIZE),GFP_KERNEL);
	cprm->pbbuf_base = (cc_u8*)kmalloc((SDRM_RSA_ALLOC_SIZE),GFP_KERNEL);
	cprm->pbbuf_out = (cc_u8*)kmalloc((SDRM_RSA_ALLOC_SIZE),GFP_KERNEL);
	cprm->mod    = SDRM_BN_Alloc((cc_u8*)cprm->pbbuf_mod, SDRM_RSA_BN_BUFSIZE);
	cprm->exp    = SDRM_BN_Alloc((cc_u8*)cprm->pbbuf_exp, SDRM_RSA_BN_BUFSIZE);
	cprm->base   = SDRM_BN_Alloc((cc_u8*)cprm->pbbuf_base, SDRM_RSA_BN_BUFSIZE);
	cprm->out = SDRM_BN_Alloc((cc_u8*)cprm->pbbuf_out, SDRM_RSA_BN_BUFSIZE);
	cprm->bufs_index = 0;
	SDRM_OS2BN(GN_Coredump, RSA_KEYBYTE_SIZE, cprm->mod);
...
static int free_workspace(struct coredump_params *cprm)
{
...
static int compress_coredump(struct file *file, unsigned char *uncomp_src, unsigned char *comp_buf,
		loff_t uncomp_src_len, unsigned long *crc, u32 *is_z_finish,struct coredump_params *cprm)
{
	int ret = 0;
	...
	cprm->def_strm.next_in = uncomp_src;
	cprm->def_strm.total_in = 0;

	cprm->def_strm.next_out = comp_buf;
	cprm->def_strm.total_out = 0;


	cprm->def_strm.avail_out = uncomp_src_len;
	cprm->def_strm.avail_in = uncomp_src_len;

	/* crc32 check, used in GZIP tailer */
	*crc = gzip_crc32_le(cprm->def_strm.next_in, cprm->def_strm.avail_in);
	...
	encrypt_write(file,comp_buf,cprm->def_strm.total_out,ENCODE_IN_SIZE,cprm);	
	...
	prev_comp_write_offset = cprm->def_strm.total_out;

	/* End of compressed data block, deflate Z_FINISH */
	if (*is_z_finish) {
		coredump_debug("##### (vma->vm_next) == NULL ...\n");
		cprm->def_strm.next_out = comp_buf;
		cprm->def_strm.avail_out += STREAM_END_SPACE;
		cprm->def_strm.avail_in = 0;
		...
		comp_write_offset = (cprm->def_strm.total_out - prev_comp_write_offset);

	encrypt_write(file,comp_buf,comp_write_offset,ENCODE_IN_SIZE,cprm);	
	}
...
#define CORE_SMALL_VMA          2
    ...
    /* Align to page */
...
#ifdef CONFIG_ENCRYPT_COREDUMP
       create_workspace(cprm);
       if (set_gzip_header(cprm->file,cprm) < 0) {
#else
...
#ifdef CONFIG_ENCRYPT_COREDUMP
       if (coredump_alloc_workspaces(&cprm->def_strm) < 0) {
#else
       ...
       /* gzip deflate init */
#ifdef CONFIG_ENCRYPT_COREDUMP
       if (Z_OK != zlib_deflateInit2(&cprm->def_strm, 8, Z_DEFLATED, -MAX_WBITS,
...
#ifdef CONFIG_ENCRYPT_COREDUMP
                       aligned_elf_foffset, &crc32_val, &z_finish,cprm);
#else
...
#ifdef CONFIG_ENCRYPT_COREDUMP
                                               ULTIMATE_COMP_BUF_SIZE, &crc32_val, &z_finish,cprm);
#else
               ...
               else if (buf_usage_cnt == 1)
               {
                       ...
                       switch (vma_type)
                       {
...
#ifdef CONFIG_ENCRYPT_COREDUMP
                                       ret_comp_val = compress_coredump(cprm->file, uncomp_src_buf, compressed_buf,vma_size,&crc32_val, &z_finish,cprm);
#else
                               ...
                               case CORE_BIG_VMA:
                                       ...
                                       ret_comp_val = compress_coredump(cprm->file, uncomp_src_buf, compressed_buf,
#ifdef CONFIG_ENCRYPT_COREDUMP
                                                       vma_size - ULTIMATE_COMP_BUF_SIZE*is_buf_full_cnt, &crc32_val, &z_finish,cprm);
#else
...
#ifdef CONFIG_ENCRYPT_COREDUMP
    if (set_gzip_tailer(cprm->file, &crc32_val, &uncomp_coredump_file_size,cprm) < 0)
#else
...
#ifdef CONFIG_ENCRYPT_COREDUMP
    coredump_free_workspaces(&cprm->def_strm);  
    free_workspace(cprm);
#else	
