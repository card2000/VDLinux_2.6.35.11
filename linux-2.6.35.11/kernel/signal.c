...
#if 1  /* ignored user defined signal handler, call default handler, Linux Part 2010-11-24 */
			if ((signr == SIGBUS) ||  (signr == SIGABRT) || (signr == SIGSEGV) || (signr == SIGILL)) {
				printk(KERN_ALERT " ##### ignored user defined signal (%d) handler, call Coredump \n", signr); 
			} else {
				printk(KERN_ALERT " ##### call default signal (%d) handler\n", signr); 
				break; /* will return non-zero "signr" value */
			}
#endif
		}
...
#ifdef CONFIG_PRINT_KILL_SIGNAL 
       if( tsk != NULL ){
               printk(KERN_ALERT "##### send signal from USER, SIG : %d, %s(%d)->%s(%d) %s\n",
                               sig, current->comm, current->pid, tsk->comm, tsk->pid, __func__);
	}
       else{
               printk(KERN_ALERT "##### send signal from USER, SIG : %d, %s(%d)->???(%d) %s\n",
                               sig, current->comm, current->pid, pid, __func__);
	}
