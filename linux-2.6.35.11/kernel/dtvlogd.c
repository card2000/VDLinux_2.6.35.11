...
int dtvlogd_write_start_stop = 1; /* Flag to write DTVLOGD Buffer */
#endif
 ...
 *       __DRAM_BUF_LEN is defeind for this.
 *
...
#error "CONFIG_DTVLOGD_EMEG_LOGD_BUFFER_ADDRESS \
		isn't aligned memory with 0xFFFF"
#endif

#define RAM_DATA_ADDR(buf_addr)  (buf_addr + 0x10000)
...
void dtvlogd_usblog_start(char *mount_dir)
{
	...
	strncat(logname, "/log.txt", LOGNAME_SIZE - len);
		...
		size_t count, loff_t *ppos)
{
	...
	buffer[MAX_DMSG_WRITE_BUFFER-1] = '\0';
