			  ...
			  unsigned long alignment, bool init_migratetype)
{
	...
	pgdat = NODE_DATA(0);
	if (pgdat)
		_max_low_pfn = pgdat->bdata->node_low_pfn;

	if (phys_to_pfn(start) >= _max_low_pfn)
		goto init;

	/* Reserve memory */
	if (start) {
		if (reserve_bootmem(start, size, BOOTMEM_DEFAULT) < 0)
		 ...
		 */
		addr = (unsigned long)
			__alloc_bootmem_nopanic(size, alignment, 0);
...
#ifdef CONFIG_CMA_PREVENT_MIGRATE_OOM
#define CMA_MIGRATE_PAGES_THRESHOLD CONFIG_CMA_MIGRATE_PAGES_THRESHOLD

static unsigned long cma_get_lru_pages_num(unsigned long start,
						unsigned long end)
{
...
static int cma_check_migration_possible(unsigned long start, unsigned long end)
{
	int ret = 0;
	int i;
	unsigned long reserve_max = 0;
	struct zone *zone;
	unsigned long free_pages = 0;
	unsigned long zone_free_pages = 0;
	unsigned long lru_pages = cma_get_lru_pages_num(start, end);

	for_each_zone(zone) {
		if (is_highmem(zone) || is_normal(zone)) {
			reserve_max = 0;
			for (i = 0; i < ARRAY_SIZE(zone->lowmem_reserve); i++) {
				if (reserve_max < zone->lowmem_reserve[i])
					reserve_max = zone->lowmem_reserve[i];
			}
			zone_free_pages = zone_page_state(zone, NR_FREE_PAGES);
			if (zone_free_pages > reserve_max +
					high_wmark_pages(zone))
				free_pages += zone_free_pages - reserve_max -
							high_wmark_pages(zone);
		}
	}
	/* Don't account free_pages in [start, end] region */
	free_pages -= end - start - lru_pages;

	if (free_pages < lru_pages + CMA_MIGRATE_PAGES_THRESHOLD)
		ret = lru_pages + CMA_MIGRATE_PAGES_THRESHOLD - free_pages;
...
#ifdef CONFIG_CMA_PREVENT_MIGRATE_OOM
		ret = cma_check_migration_possible(pfn,
					pfn + (size >> PAGE_SHIFT));
		if (ret > 0) {
			if (failed_pages) {
				*failed_pages = ret;
