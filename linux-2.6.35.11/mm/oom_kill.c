...
#ifdef CONFIG_RAMZSWAP
extern int show_ramzswap_info(void);
#endif
...
static inline void dump_tasks_plus(const struct mem_cgroup *mem,struct seq_file *s)
{
	...
	do_each_thread(g, p) {
		...
		display_dtp_counters(s,p,mm,&vmem,&rss,&pss,&shared,&private,&swap);
...
static
#endif
void dump_tasks(const struct mem_cgroup *mem,struct seq_file *s)
{
	...
	do_each_thread(g, p) {
		...
		if (s)
			seq_printf(s,"[%5d] %5d %5d %8lu %8lu %8lu %6lu %3d     %3d %s\n",
		       p->pid, __task_cred(p)->uid, p->tgid, K(mm->total_vm),
		       K(tot_rss_cnt), K(tot_maxrss_cnt), K(get_mm_counter(mm, MM_SWAPENTS)), (int)task_cpu(p), p->signal->oom_adj,
		       p->comm);
		else
		...
		sum_of_tot_swap_cnt += get_mm_counter(mm, MM_SWAPENTS);
		tot_rss_cnt = 0;
		tot_maxrss_cnt = 0;
#endif
		task_unlock(p);
