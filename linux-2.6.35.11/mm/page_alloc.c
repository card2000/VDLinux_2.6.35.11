...
#if defined(CONFIG_MIGRATE_CMA) && !defined(CONFIG_ENABLE_GFP_CMA)
...
again:
#ifdef CONFIG_ENABLE_GFP_CMA
	if (likely(order == 0) && !(gfp_flags&__GFP_CMA)) {
#else
	if (likely(order == 0)) {
#endif
		struct per_cpu_pages *pcp;
		...
		if (migratetype == MIGRATE_MOVABLE && (gfp_flags&__GFP_CMA))
			page = __rmqueue(zone, order, MIGRATE_CMA);
		if (unlikely(!page))
