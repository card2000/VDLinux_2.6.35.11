...
#define BOOTCHART_VER_STR   "3.0.1"
...
inline static int sec_bp_log_ds_entry(struct gendisk* gd, struct disk_part_iter* piter);
...
inline static int sec_bp_ds_filter(const char* str)
{
	...
	return ((str[0] == 't' && str[1] == 'b') /* tbml... */
			...
			/* For tegra */
			|| (str[0] == 'l' && str[1] == 'o' && str[2] == 'o' && str[3] == 'p'));
#endif
}
...
inline static int sec_bp_log_ds_entry(struct gendisk* gd, struct disk_part_iter* piter)
{
	...
	//preempt_disable();
	if ((hd = disk_part_iter_next(piter))) {
		cpu = part_stat_lock();
		part_round_stats(cpu, hd);
		part_stat_unlock();
	}
...
int usb_detect(char * usb_path, int max_path_len)
{
	...
	if((bytesread = filp->f_op->read(
					filp,read_buf,READ_BUF_SIZE,&filp->f_pos)) <= 0 ) {
	 ...
	 * started  with  "/dev/sd"  (usb  make dev node in /dev/sd** ) 
	 * the format of that file is 
	 * "devnode"  "moount dir" "option" ...............
	 * by this way we can find the directory where usb is mounted 
	 */
