...
#define EMAC_CHECK_CNT              (0x100000)
...
static struct tx_ring* tx_ring_get (struct EMAC_private *LocPtr, u8 bDel);
static struct tx_ring* tx_ring_add (struct EMAC_private *LocPtr, struct sk_buff *skb);
...
{
    struct EMAC_private *LocPtr = priv;
    u32 ethcmd;
    int res = 0;
    unsigned long flags;
...
static int MDev_EMAC_CheckTSR(void)
{
    ...
    for (; check < EMAC_CHECK_CNT; check++)
    {
        tsrval = MHal_EMAC_Read_TSR();
        if(tsrval & EMAC_IDLETSR)
            avlfifo[0] = 1;
        if(tsrval & EMAC_BNQ)
            avlfifo[1] = 1;
        if(tsrval & EMAC_TBNQ)
        ...
        if(tsrval & EMAC_FBNQ)
            avlfifo[3] = 1;
        if(tsrval & EMAC_FIFO1IDLE)
            avlfifo[4] = 1;
        if(tsrval & EMAC_FIFO2IDLE)
            avlfifo[5] = 1;
        if(tsrval & EMAC_FIFO3IDLE)
            avlfifo[6] = 1;
        if(tsrval & EMAC_FIFO4IDLE)
            avlfifo[7] = 1;
...
{
	struct tx_ring* tx_fifo = NULL;
    struct EMAC_private *LocPtr = priv;

    if (NETDEV_TX_OK != MDev_EMAC_CheckTSR())
        return NETDEV_TX_BUSY;

	//resend all queued packet
    while (NULL != (tx_fifo = tx_ring_get(LocPtr, 0)))
    {
        //Set address of the data in the Transmit Address register //
	    MHal_EMAC_Write_TAR(tx_fifo->skb_physaddr);
        // Set length of the packet in the Transmit Control register //
        MHal_EMAC_Write_TCR(tx_fifo->skb->data_len);
    }
    return NETDEV_TX_OK;
}
...
static struct tx_ring* tx_ring_add (struct EMAC_private *LocPtr, struct sk_buff *skb)
{
    struct tx_ring* ret = NULL;

    //check if ring fifo is full
    if ((LocPtr->tx_wridx+1)%TX_RING_SIZE == LocPtr->tx_rdidx)
    {
        //printk("FIFO is full wr:%u rd:%d test:%u\n", LocPtr->tx_wridx, LocPtr->tx_rdidx,test);
	    return NULL;
    }

    LocPtr->tx_fifo[LocPtr->tx_wridx].skb= skb;
    LocPtr->tx_fifo[LocPtr->tx_wridx].used = 1;
    ret = &(LocPtr->tx_fifo[LocPtr->tx_wridx]);
    //printk("Add skb addr:%x idx:%u\n", (u32)skb, LocPtr->tx_wridx);
    LocPtr->tx_wridx++;
    if (LocPtr->tx_wridx == TX_RING_SIZE)
        LocPtr->tx_wridx = 0;

    return ret;
}

static struct tx_ring* tx_ring_get (struct EMAC_private *LocPtr, u8 mode)
{
     struct tx_ring* ret = NULL;

    //check if ring fifo is empty
    if (LocPtr->tx_wridx == LocPtr->tx_rdidx)
    {
//        printk("FIFO is empty wr:%u rd:%d\n", LocPtr->tx_wridx, LocPtr->tx_rdidx);
	    return ret;
    }

    if (!LocPtr->tx_fifo[LocPtr->tx_rdidx].used)
    {
	    printk("Tx fifo get error idx:%u wr:%u rd:%d\n", LocPtr->tx_rdidx, LocPtr->tx_wridx, LocPtr->tx_rdidx);
    }
    ret = &(LocPtr->tx_fifo[LocPtr->tx_rdidx]);
    if (mode > 0)
    {
	    LocPtr->tx_fifo[LocPtr->tx_rdidx].used = 0;
	    LocPtr->tx_rdidx++;
	    if (LocPtr->tx_rdidx == TX_RING_SIZE)
	        LocPtr->tx_rdidx = 0;
    }
...
static int MDev_EMAC_tx (struct sk_buff *skb, struct net_device *dev)
{
    ...
    if (NULL == (tx_fifo = tx_ring_add(LocPtr, skb)))
    {
   	    netif_stop_queue (dev);
        spin_unlock_irqrestore(LocPtr->lock, flags);
        return NETDEV_TX_BUSY;
...
#ifndef TX_SKB_PTR
    #ifndef TX_QUEUE_4
    tx_fifo->skb_physaddr = EMAC_TX_PTK_BASE;
    memcpy((void*)tx_fifo->skb_physaddr, skb->data, skb->len);
    #else
    //tx_fifo->skb_physaddr = dma_map_single(NULL, skb->data, skb->len,DMA_TO_DEVICE);
    tx_fifo->skb_physaddr = get_tx_addr();
    memcpy((void*)tx_fifo->skb_physaddr, skb->data, skb->len);
    //printk("tx_fifo->skb_physaddr:%x \n", tx_fifo->skb_physaddr);
    #endif
#else
    LocPtr->txpkt = dma_map_single(NULL, skb->data, skb->len,DMA_TO_DEVICE);
#endif

    if (!tx_fifo->skb_physaddr)
    {
        dev_err(NULL,
...
#ifdef CHIP_FLUSH_READ
    #if defined(CONFIG_MIPS)
    if((unsigned int)tx_fifo->skb_physaddr < 0xC0000000)
    {
        Chip_Flush_Memory_Range((unsigned int)tx_fifo->skb_physaddr&0x0FFFFFFF, skb->len);
    }
    else
    {
        Chip_Flush_Memory_Range(0, 0xFFFFFFFF);
            ...
            Chip_Flush_Memory_Range(0, 0xFFFFFFFF); //flush miu pipe only
            //Chip_Flush_Cache_Range((unsigned int)tx_fifo->skb_physaddr, skb->len);
        #else
            Chip_Flush_Cache_Range((unsigned int)tx_fifo->skb_physaddr, skb->len);
        #endif
    #else
    ...
    //Set address of the data in the Transmit Address register //
    MHal_EMAC_Write_TAR(BUS2PA(tx_fifo->skb_physaddr - RAM_VA_PA_OFFSET));
    // Set length of the packet in the Transmit Control register //
...
static int MDev_EMAC_rx (struct net_device *dev)
{
    ...
    #ifdef RX_SOFTWARE_DESCRIPTOR//SOFTWARE_DESCRIPTOR
...
irqreturn_t MDev_EMAC_interrupt(int irq,void *dev_id)
{
    ...
    oldTime = getCurMs();
    while((xReceiveFlag == 1) || (xReceiveNum != 0) ||  (intstatus = (MHal_EMAC_Read_ISR() & ~MHal_EMAC_Read_IMR() & MASK )) )
    {
        ...
        if (intstatus & EMAC_INT_TCOM)
        {
            ...
            if (intstatus & (EMAC_INT_TUND | EMAC_INT_RTRY))
            {
            ...
            #if 0           //To do after u06
                if (LocPtr->stats.tx_errors == 1)
                {
                    EMAC_DBG("==> Set EMAC MIU priority to highest (0x1288 bit 12 -> 0)\n");
                    MHal_EMAC_Write_JULIAN_0100(JULIAN_100_VAL|0x00080000);
                }
            #endif
                if(intstatus & EMAC_INT_TUND)
                {
            	    ...
            	    {
            	        LocPtr->retx_count++;
    		            //EMAC_DBG("==> %s: Resend TUND packet %u times!\n", dev->name, LocPtr->retx_count);
                        goto rx;
            	    }
...
irqreturn_t MDev_EMAC_interrupt(int irq,void *dev_id)
{
    ...
    while(intstatus = (MHal_EMAC_Read_ISR() & ~MHal_EMAC_Read_IMR() & MASK))
    {
...
static u32 MDev_EMAC_VarInit(void)
{
...
#endif
    memset((u32 *)alloRAM_VA_BASE,0x00,EMAC_ABSO_MEM_SIZE);
    ...
    printk("RAM_VA_BASE:%x\n", RAM_VA_BASE);
    printk("RAM_PA_BASE:%x\n", RAM_PA_BASE);
    printk("RX_BUFFER_BASE:%x\n", RX_BUFFER_BASE);
    printk("RBQP_BASE:%x\n", RBQP_BASE);
    printk("TX_BUFFER_BASE:%x\n", TX_BUFFER_BASE);
    printk("RAM_VA_PA_OFFSET:%x\n", RAM_VA_PA_OFFSET);
    printk("TX_SKB_BASE:%x\n", TX_SKB_BASE);
...
static int MDev_EMAC_setup (struct net_device *dev, unsigned long phy_type)
{
    ...
    priv = (struct EMAC_private *) RetAddr;//alloRAM_ADDR_BASE;
    ...
    priv->dlist_phys = priv->dlist = (struct recv_desc_bufs *) (RBQP_BASE + RAM_VA_PA_OFFSET);

    if (priv->dlist == NULL)
    {
        dma_free_noncoherent(&priv,EMAC_ABSO_MEM_SIZE,&dmaaddr,0);//kfree (&priv);
        free_irq (dev->irq, dev);
        return -ENOMEM;
    }
    ...
    if ((val & (1 << 10)) == 0)   // DSCR bit 10 is 0 -- fiber mode //
        priv->phy_media = PORT_FIBRE;

    spin_unlock_irqrestore(priv->lock, flags);

    //Support for ethtool //
    priv->mii.dev = dev;
    priv->mii.mdio_read = MDev_EMAC_mdio_read;
    priv->mii.mdio_write = MDev_EMAC_mdio_write;
...
static int MDev_EMAC_SwReset(struct net_device *dev)
{
    ...
    netif_stop_queue (dev);
    MHal_EMAC_Write_JULIAN_0100(JULIAN_100_VAL & 0x00000FFF);
    MHal_EMAC_Write_JULIAN_0100(JULIAN_100_VAL);
