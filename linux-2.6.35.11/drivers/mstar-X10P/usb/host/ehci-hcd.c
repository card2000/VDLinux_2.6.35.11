...
static int ehci_run (struct usb_hcd *hcd)
{
	...
	if (hcd->self.busnum == 1)
	{
		writeb(readb((void*)(_MSTAR_UHC1_BASE+0x81*2-1)) | 0xC0,(void*)(_MSTAR_UHC1_BASE+0x81*2-1));
	}
	else if (hcd->self.busnum == 2)
	{
		writeb(readb((void*)(_MSTAR_UHC_BASE+0x81*2-1)) | 0xC0,(void*)(_MSTAR_UHC_BASE+0x81*2-1));
	}
#endif
...
#if 0	//tony add for disconnect adjust
	if(UHC_getPortSC(hcd->self.busnum) & 0x01){
		if (bh)
			ehci_work (ehci);
	}
#else
...
/* tony.yu force not configure CONFIG_PCI in AMBER3 */
#if defined(CONFIG_PCI) && !defined(CONFIG_MSTAR_AMBER3)
#include "ehci-pci.c"
...
//module_init(second_ehci_hcd_init);
...
//module_exit(second_ehci_hcd_cleanup);
