...
extern unsigned char hcd_readb(struct usb_hcd *, void *);
extern void hcd_writeb(struct usb_hcd *, unsigned char , void *);

int UHC_getPortSC(struct usb_bus *bus)
{
	int tmp=0;
	...
	if(busnum == 2) {
		tmp=hcd_readb(hcd, (void*)(_MSTAR_UHC0_BASE+0x30*2));
	}
	else if(busnum == 1) {
		tmp=hcd_readb(hcd, (void*)(_MSTAR_UHC1_BASE+0x30*2));
	}
	#ifdef ENABLE_THIRD_EHC
	else if(busnum == 3) {
		tmp=hcd_readb(hcd, (void*)(_MSTAR_UHC2_BASE+0x30*2));
	}
	#endif
