...
#ifdef SAMSUNG_PATCH_WITH_NOT_SUPPORTED_DEVICE
...
#ifdef SAMSUNG_PATCH_WITH_NOT_SUPPORTED_DEVICE
	//for usb2.0 certification
	if(ReadUsbClass(dev) == 1)
	{
		printk("\tnot supported device connected \tManufacturer: %s \t Product : %s\n", dev->manufacturer,dev->product);
		kobject_uevent(&dev->class_dev->kobj, KOBJ_NOTSUPPORTED_DEV_ADD);
	}
#endif
	return 0;
}
...
{
#ifdef SAMSUNG_PATCH_WITH_NOT_SUPPORTED_DEVICE
	printk(" \tManufacturer: %s \t Product : %s\n", dev->manufacturer,dev->product);
#endif
