...
unsigned char bouncing[2048] __attribute((aligned(64)));
...
extern wait_queue_head_t onif_wait;
extern int onif_int;
// Added by Mstar, external variable declaration.
extern U8 onif_direct_read;
...
unsigned short MDrv_ONENAND_read_word(void __iomem *addr)
{
...
#elif defined(CONFIG_MSTAR_TITANIA8)|| defined(CONFIG_MSTAR_TITANIA9)
	...
	HAL_ONIF_RW_Init(RW_MODE_ASYNC);
        // Added by Mstar, direct read.
	if( onif_direct_read )
	{
		u16Data = *((volatile unsigned short *)addr);
	}
	else
	{
	    ...
	    REG_WRITE_UINT16(ONIF_CMDQ_PARAM, ((U32)addr&0x1FFFF)>>1);
	    ...
	    u16Data = REG(ONIF_ASYNC_READ_DATA);
	}
...
void MDrv_ONENAND_write_word(unsigned short value, void __iomem *addr)
{
	...
	REG_WRITE_UINT16(ONIF_CMDQ_PARAM, value);
