...
#ifdef CONFIG_NVT_NT72558
extern void set_sys_mpll(unsigned long offset, unsigned long value);
#endif
...
{   
	...
	if(!gmacdev->LinkState){
		...
		status = synopGMAC_check_phy_init_for_10_100M(gmacdev);
...
//		#endif
		...
		do{
			...
			status = synopGMAC_set_rx_qptr(gmacdev,virt_to_phys(skb->data), skb_tailroom(skb), (u32)skb,0,0,0);
...
s32 synopGMAC_linux_open(struct net_device *netdev)
{
	...
	synopGMAC_set_mdc_clk_div(gmacdev,s32MDC_CLOCK);
...
/* Remove by Ann@20110810 */
/* Get HCLK from system */
#if 0
int SysPlat_AHB_Get(void)
{
	u32 cpu_mpll, hclk_sel;
	
	cpu_mpll = (*((volatile u32 *)0xbc0c1160)>>16) + (*((volatile u32 *)0xbc0c1164)<<16);
	hclk_sel = (*((volatile u32 *)0xbc0c0014)>>24)&0xf;
	if ( hclk_sel == 0 )
		return cpu_mpll*12/1024/128/6;		// ahb = cpu/6, mpll value = cpu/12 * 2(17)
	else if ( hclk_sel == 1 )
		return cpu_mpll*12/1024/128/4;		// ahb = cpu/4, mpll value = cpu/12 * 2(17)
	else
		return cpu_mpll*12/1024/128/4;		// not support, mpll value = cpu/12 * 2(17)
}
#endif
...
void SysPlat_Init_Ethernet_Phy(void)
{
	...
	/* Disable by Ann@20110810 */
#ifndef CONFIG_NVT_NT72558
#define QFP_BOARD 0
#if QFP_BOARD
	// jay hsu : set pin mux to mac
	*((volatile u32 *)0xbc0d013c) |= 0x100;
#else
	*((volatile u32 *)0xbc0d013c) &= (~0x100);
	//*((volatile u32 *)0xbc0d013c) = 0;	
#endif
#endif

	//printk("jay debug : pin mux 0x%08x\n", *((volatile u32 *)0xbc0d013c));
	// jay hsu : set Mac mpll speed to 50MHz
	/* Modify by Ann@20110810 */
	/* MPLL setting */
#ifdef CONFIG_NVT_NT72558
	set_sys_mpll(_ETH_PLL_OFFSET, 0x085555);
	...
	*((volatile u32 *)0xbd020044) |= 0x00000040;
	...
	/* GPIO default enable */
#else
	*((volatile u32 *)0xbc0c11a8) = 0x55550000;
	*((volatile u32 *)0xbc0c11ac) = 0x00000008;
	// jay hsu : enable Mac
	*((volatile u32 *)0xbc0d0000) |= 0x40;
	*((volatile u32 *)0xbc0d0000) |= 0x40;
	//wbflush_ahb();	// flush write register
	// jay hsu : enable gpio
	*((volatile u32 *)0xbc0d0000) |= 0x1000000;

	/* MAC clock inverse */
	*((volatile u32 *)0xBC0D0030) |= 0x00000002;
...
#else
#define PHY_RESET_GPIO	5
#endif
#endif
	// jay hsu : ethernet phy reset
#ifdef CONFIG_NVT_NT72558
	*((volatile u32 *)0xbd101408) |= (0x00000001 << PHY_RESET_GPIO);
	*((volatile u32 *)0xbd101400) |= (0x00000001 << PHY_RESET_GPIO);

	mdelay(1);		// jay hsu : delay 1 msec at least to let phy latch the strap pins

	*((volatile u32 *)0xbd101404) |= (0x00000001 << PHY_RESET_GPIO);
#else
	*((volatile u32 *)0xbd0d0000) |= (1<<PHY_RESET_GPIO);
	*((volatile u32 *)0xbd0d0004) |= (1<<PHY_RESET_GPIO);
	*((volatile u32 *)0xbd0d0004) &= (~(1<<PHY_RESET_GPIO));
	mdelay(1);		// jay hsu : delay 1 msec at least to let phy latch the strap pins
	*((volatile u32 *)0xbd0d0004) |= (1<<PHY_RESET_GPIO);	
#endif
