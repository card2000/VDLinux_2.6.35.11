...
int auto_mdix_mode=TRUE;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0))
    module_param(auto_mdix_mode, bool, 0);
#else
    MODULE_PARM(auto_mdix_mode,"bool");
#endif
MODULE_PARM_DESC(auto_mdix_mode,"Set automdix mode");
...
static int smsc9500_reset(struct usbnet *dev)
{
	...
	{
		dwAddrH=0x00000000UL;
		dwAddrL=0x00000000UL;
...
static int smsc9500_eth_phy_boost(struct usbnet *dev, int mode)
{
	...
	if ((dev->chipID == PID_LAN9730) || (dev->chipID == PID_LAN9530) || (dev->chipID == PID_LAN89530)) {
		if (mode == 1) {
			/* set boost */
