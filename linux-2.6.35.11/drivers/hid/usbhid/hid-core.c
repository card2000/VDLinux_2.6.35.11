...
static void hid_reset(struct work_struct *work)
{
	...
	{
        if (hid_to_usb_dev(hid)->config->desc.bNumInterfaces > 1)
        {
            err_hid("skip clearing halt, because the number of interface is %d", hid_to_usb_dev(hid)->config->desc.bNumInterfaces);
        }
        else
        {
...
#if defined(CONFIG_MACH_FIRENZE)
	if (dev->descriptor.idVendor == 0x05ac) {
		if ((dev->descriptor.idProduct >> 8) == 0x12)
			return -ENODEV ;
		if ((dev->descriptor.idProduct >> 8) == 0x13)
			return -ENODEV ;
	}
#endif
