...
static void NT725581_thread(unsigned long param)
{
	dirty_writeback_interval = 500;
}

static void NT725581_stop(struct platform_device *dev)
{
	writel(0x2, USB1_EHCI_BASE+0x10);
	udelay(1000);

	writel(0x40,0xbd020064);
	mdelay(1);
	writel(0x40,0xbd020060);

	writel(0x4,0xbd0200a4);
	mdelay(1);
	writel(0x4,0xbd0200a0);

	writel(0x8,0xbd020084);
	mdelay(1);
	writel(0x8,0xbd020080);
	
}

/*-------------------------------------------------------------------------*/

void NT725581_init(void)
{
	//# set uclk sw reset
	writel(0x40,0xbd020064);
	mdelay(1);
	...
	writel(0x3404,0xbd110808);
	mdelay(1);
	writel(0xc04,0xbd110808);
	mdelay(1);
...
/**
 * usb_ehci_NT725581_probe - initialize NT725581-based HCDs
 ...
 *
 */
int usb_ehci_NT725581_probe(const struct hc_driver *driver,
			  struct usb_hcd **hcd_out, struct platform_device *dev)
{
	...
	hcd = usb_create_hcd(driver, &dev->dev, "NT725581");
	if (!hcd)
		return -ENOMEM;
	hcd->rsrc_start = dev->resource[0].start;
	hcd->rsrc_len = dev->resource[0].end - dev->resource[0].start + 1;

	if (!request_mem_region(hcd->rsrc_start, hcd->rsrc_len, "ehci_hcd_NT725581")) {
...
void usb_ehci_NT725581_remove(struct usb_hcd *hcd, struct platform_device *dev)
{
	usb_remove_hcd(hcd);
	release_mem_region(hcd->rsrc_start, hcd->rsrc_len);
	usb_put_hcd(hcd);
...
static void NT725581_patch(struct usb_hcd *hcd){
...
static int NT725581_ehci_init(struct usb_hcd *hcd)
{
	struct ehci_hcd		*ehci = hcd_to_ehci(hcd);
	u32			temp;
	int			retval;
	...
	init_timer(&hcd->pt);
	hcd->pt.function = NT725581_thread;
...
static const struct hc_driver ehci_NT725581_hc_driver = {
	.description =		hcd_name,
	.product_desc =		"Novatek NT725581",
	...
	.port_nc = NT725581_patch,
};


/*-------------------------------------------------------------------------*/

static int ehci_hcd_NT725581_drv_probe(struct platform_device *pdev)
{
	...
	ret = usb_ehci_NT725581_probe(&ehci_NT725581_hc_driver, &hcd, pdev);
	return ret;
}

static int ehci_hcd_NT725581_drv_remove(struct platform_device *pdev)
{
	struct usb_hcd *hcd = platform_get_drvdata(pdev);
	pr_debug("In ehci_hcd_NT725581_drv_remove\n");
	usb_ehci_NT725581_remove(hcd, pdev);
	platform_set_drvdata(pdev, NULL);

	return 0;
}

MODULE_ALIAS("NT725581-ehci");
static struct platform_driver ehci_hcd_NT725581_driver = {
	.probe = ehci_hcd_NT725581_drv_probe,
	.remove = ehci_hcd_NT725581_drv_remove,
	.driver = {
		.name = "NT725581-ehci",
