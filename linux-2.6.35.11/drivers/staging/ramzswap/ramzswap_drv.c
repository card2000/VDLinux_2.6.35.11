...
static unsigned int num_devices = CONFIG_RAMDEVICES;
...
int show_ramzswap_info(void);
int show_ramzswap_info(void)
{
	size_t dev_id, cur_dev_id = 0;	/* curent used dev_id is 0 */
	size_t mem_used = 0;
	struct ramzswap *rzs = NULL;
	struct ramzswap_stats *stats = NULL;

	for (dev_id=0; dev_id <= cur_dev_id; dev_id++) {
		rzs = &devices[cur_dev_id];

		if (rzs == NULL) {
			printk(KERN_NOTICE"ramzswap device ID : %u is NULL, check device ID\n", dev_id);
			return -EINVAL;
		}
		...
		printk(KERN_NOTICE"VDLinux msg, ramzswap%u info \n", dev_id);
		printk(KERN_NOTICE"ZeroDataSize: %10u kB\n" "OrigDataSize: %10u kB\n" 
				"  ComprDataSize: %7u kB\n" "  UnComprDataSize: %5u kB\n" 
				"MemUsedTotal:  %9u kB\n",
				(stats->pages_zero << PAGE_SHIFT) >> 10,
				(stats->pages_stored << PAGE_SHIFT) >> 10,
				stats->compr_size >> 10, 
				(stats->pages_expand << PAGE_SHIFT) >> 10,
				mem_used >> 10);
	}
