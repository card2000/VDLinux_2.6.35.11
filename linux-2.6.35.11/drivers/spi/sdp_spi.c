...
/* clock func is not exported. so clock func def const value when build in Mudule*/
#ifdef MODULE
...
static unsigned int sdp1106_calc_bus_pms(unsigned int pms)
{
	unsigned int ret;
...
static struct clk* sdp1106_spi_get_pclk(void)
{
	dummy_pclk.rate = sdp1106_calc_bus_pms(R_PMU_BUS_PMS_CON) / 2;
	return &dummy_pclk;
}

#define clk_disable(clk) 
#define clk_enable(clk)		(0)
#define clk_get(dev, id)	sdp1106_spi_get_pclk()
#define clk_put(clk) 
#define clk_get_rate(clk)	(((clk)->rate) ? (clk)->rate : (clk)->source->rate)
...
#define DEBUG 1
#define SDPSPI_DBG(dev, fmt, arg...) dev_dbg(dev, fmt, ## arg)
///////////////////////////////
/**
...
sdp_spi_setup(struct spi_device *spi)
{
	...
	if (!chip) {
		SDPSPI_DBG(&sdpspi->pdev->dev, "initial setup for %s\n",
			spi->modalias);
	...
	if (spi->max_speed_hz != chip->rate) {
		...
		if (err != 0) {
			SDPSPI_DBG(&sdpspi->pdev->dev, "error in calc_divisors.\n");
	...
	if(unlikely(chip->frame_format > 2) ) {
		SDPSPI_DBG(&sdpspi->pdev->dev, "invalied frame format.\n");
		return -EINVAL;
	}
				  ...
				  const struct sdp_spi_chip *chip)
{
	...
	SDPSPI_DBG(&sdpspi->pdev->dev, "sdp_spi_chip_setup: Regdump CR0=%#x, CR1=%#x, SSPCPSR=%#x\n", cr0, cr1, chip->div_cpsr);
...
sdp_do_write(struct sdp_spi *sdpspi, struct spi_transfer *t)
{
	if (bits_per_word(sdpspi) > 8) {
		...
		SDPSPI_DBG(&sdpspi->pdev->dev, "sdp_do_write Data 0x%x\n", tx_val);
	} else {
		u8 tx_val = 0;
...
sdp_do_read(struct sdp_spi *sdpspi, struct spi_transfer *t)
{
	if (bits_per_word(sdpspi) > 8) {
		...
		SDPSPI_DBG(&sdpspi->pdev->dev, "sdp_do_read Data 0x%x\n", rx_val);
	} else {
		u8 rx_val;
...
sdp_spi_read_write(struct sdp_spi *sdpspi)
{
	...
	struct spi_transfer *t = msg->state;
	/*
	SDPSPI_DBG(&sdpspi->pdev->dev, "## fifo_level%d, sdpspi->rx%d, sdpspi->tx%d, t->len%d\n",
...
//		goto fail_put_clock;
	...
	dev_info(&pdev->dev, "SPI Support Slave Clock [Max %ld - Min %ld]\n",
		sdpspi->max_rate, sdpspi->min_rate);
...
sdp_spi_init(void)
{
	...
	//Pad control reg bit 15 set SPI use
	tmp = readl((REG_PAD_CTRL + DIFF_IO_BASE0));
	tmp |= PAD_CTRL_SELSPI;
	writel(tmp, (REG_PAD_CTRL + DIFF_IO_BASE0));

#elif defined(CONFIG_ARCH_SDP1004)	//for Firenze
	//if used o_SPI_nSSI then 656_IN_SEL and AUDIO_JTAG_SEL is not select.
	tmp = readl((REG_SUB_FUNC_SEL + DIFF_IO_BASE0));
	tmp &= ~(SUB_FUNC_SEL_656_IN_SEL | SUB_FUNC_SEL_AUDIO_JTAG_SEL);
	writel(tmp, (REG_SUB_FUNC_SEL + DIFF_IO_BASE0));
	...
	tmp |= MSPI_TX_CTRL_SELSPI;
	writel(tmp, (REG_MSPI_TX_CTRL + DIFF_IO_BASE0));
	//Pad control reg bit 20 set SPI use
	tmp = readl((REG_PAD_CTRL + DIFF_IO_BASE0));
	tmp |= PAD_CTRL_SELSPI;
	writel(tmp, (REG_PAD_CTRL + DIFF_IO_BASE0));
