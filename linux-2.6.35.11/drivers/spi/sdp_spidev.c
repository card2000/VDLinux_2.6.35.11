...
spidev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SPI_IOC_WR_WHOLE_CONFIG:
	{
		...
		if (retval == 0) {
			...
			save.spi_mode = spi->mode & (SPI_CPHA|SPI_CPOL);
			save.frame_format = chip->frame_format;

			spi->max_speed_hz = new_val.max_speed_hz;
			spi->bits_per_word = new_val.bits_per_word;
			spi->mode &= ~(SPI_CPHA|SPI_CPOL);/* bit clear */
			spi->mode |= (new_val.spi_mode & (SPI_CPHA|SPI_CPOL));/* set spi mode */
			...
			if (retval < 0) {
				...
				spi->mode &= ~(SPI_CPHA|SPI_CPOL);/* bit clear */
				spi->mode |= (save.spi_mode & (SPI_CPHA|SPI_CPOL));/* set spi mode */
...
static int __init spidev_init(void)
{
	...
	if (IS_ERR(spidev_class)) {
		...
		printk(KERN_ERR "spidev_init:class_create() returned with error code %d", PTR_ERR(spidev_class));
