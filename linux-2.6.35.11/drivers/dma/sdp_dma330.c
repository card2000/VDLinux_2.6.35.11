 ...
 */
 ...
 /*
  * edited for Echo platform by dongseok lee <drain.lee@samsung.com>
  */
		...
		size_t sg_len, unsigned long flags)
{
	...
	if(IS_ERR_OR_NULL(px))
	{
		...
		dev_err(pch->dmac->pif.dev, "error alloc_xferlist()ret %d\n", px);
	}
#endif
...
pl330_probe(struct amba_device *adev, struct amba_id *id)
{
	...
	res = &adev->res;
	request_mem_region(res->start, resource_size(res), "sdp-dma330");
