...
static struct platform_device *platform_drm_device;
...
static int __init mali_platform_drm_init(void)
{
	...
	if(!ret)
    	{
    		platform_drm_device = platform_device_alloc(DRIVER_NAME, 0);
		 if (platform_drm_device)
            		ret = platform_device_add(platform_drm_device);
        	else
            		ret = -ENOMEM;
		if (ret) 
	        {
	            platform_device_put(platform_drm_device);
...
{
	platform_device_unregister(platform_drm_device);
	platform_driver_unregister( &platform_drm_driver );
}
